...
hdfs集群服务名字：dfs.nameservicesmycluster

NameNode 别名dfs.ha.namenodes.myclusternn1,nn2, nn3

NameNode 的 PRC 监听端口dfs.namenode.rpc-address.mycluster.nn1machine1.example.com:8020dfs.namenode.rpc-address.mycluster.nn2machine2.example.com:8020dfs.namenode.rpc-address.mycluster.nn3machine3.example.com:8020

NameNode 的 HTTP 监听端口dfs.namenode.http-address.mycluster.nn1machine1.example.com:9870dfs.namenode.http-address.mycluster.nn2machine2.example.com:9870dfs.namenode.http-address.mycluster.nn3machine3.example.com:9870
...
the Java class that HDFS clients use to contact the Active NameNode.dfs.client.failover.proxy.provider.myclusterorg.apache.hadoop.hdfs.server.namenode.ha.ConfiguredFailoverProxyProvider

Importantly, when using the Quorum Journal Manager, only one NameNode will ever be allowed to write to the JournalNodes, so there is no potential for corrupting the file system metadata from a split-brain scenario. 只有允许同一时刻有一个 NameNode 往JNS 里面写数据。因为会出现过时的 NameNode 好保持着 Active 状态，或者出现假死状态。

下面配置了 sshfence 用于通过 ssh 连接目标机器，运行 kill 命令。dfs.ha.fencing.methodssshfencedfs.ha.fencing.ssh.private-key-files/home/exampleuser/.ssh/id\_rsa

JNS 编辑日志的物理磁盘地址：dfs.journalnode.edits.dir/path/to/journal/node/local/data

core-site.xml 文件：

配置 ZK 集群：ha.zookeeper.quorumnode01:2181,node02:2181,node03:2181

配置 HDFS 集群的服务域名：fs.defaultFShdfs://mycluster
